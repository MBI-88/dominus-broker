...
The **`BrokerSqs`** service (protobuf package `dominus`) exposes **Producer**, **Consumer**, and **Ack** operations. This document describes layering, validation, Redis behavior, and how tests map to that behavior.

| Layer | Location |
|-------|----------|
| gRPC handlers | `internal/infrastructure/grpc/inbound/` |
| Application | `internal/application/usecases/producer`, `.../consumer`, `.../ack` |
| Domain entity | `internal/domain/entities/message.go` (`Message`) |
...
- **DTOs** — DTO interfaces live next to each use case: `internal/application/usecases/producer/dto.go`, `.../consumer/dto.go`, `.../ack/dto.go`; protobuf requests implement these interfaces at the edge.
...
Factory: each use case exposes a constructor in its package (`producer.New`, `consumer.New`, `ack.New`) which are wired in bootstrap; the same Redis `MemoryClient` is configured with stream and consumer group from config (`Group` is created at startup).
...
| **Ack** | Signature: `Ack(ctx context.Context, ms AskDto) error`. Validates message ID format, then acknowledges | Calls `AckMessage(ctx, messageId, groupId)` → `XACK` for `(stream, group, messageId)` |
...
- **Producer** — Rejects **empty** payload before calling Redis (implementation returns an error when `len(payload) == 0`).
- **Consumer** — Delegates to `GetMessage(ctx, workerId, groupId)` on the `MemoryClient`.
- **Ack** — Validates the message ID format via `entities.Message.SetMessageId`; invalid IDs cause the use case to return an error and **not** call `AckMessage` (current implementation returns `ack.Ack invalid messageId` on validation failure).

Domain rules live on **`entities.Message.SetMessageId`** / `checkValidFormatID`; stream-shaped IDs are expected (Redis-assigned `<milliseconds>-<seq>` style) and arbitrary strings/UUIDs are rejected.
...
### Unit — application (`tests/cases/application/usecases/`)
...
-- **Ack** — Signature assertions: expects `ctx context.Context` and `AskDto`; validates stream-style IDs with `AckMessage(ctx, messageId, groupId)` success/failure; **invalid** IDs ensure **`AckMessage` is not invoked** and the use case returns a validation error.
