...
func TestNew(t *testing.T) {
	...
	b := streamserver.New(c)
	if b == nil {
		t.Fatal("StreamServer returned nil")
	}
}
...
func TestStreamServer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			output: fmt.Errorf("streamserver.StreamServer connection closed"),
		},
		{
			name: "StreamServer subscribers empty",
			setupMock: func(mc *mocks.MockBrokerClient, mdto *mocks.MockBrokerServerDto, mrq *mocks.MockBrokerRequestDto) {
				mdto.EXPECT().Context().Return(context.Background())
				mrq.EXPECT().
					GetSubscribers().
			...
			output: fmt.Errorf("streamserver.StreamServer subscribers not found"),
		},
		{
			name: "StreamServer send error then connection closed",
			setupMock: func(mc *mocks.MockBrokerClient, mdto *mocks.MockBrokerServerDto, mrq *mocks.MockBrokerRequestDto) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			serverUseCase := streamserver.New(mockClient)

			err := serverUseCase.StreamServer(mockRequestDto, mockDto)
